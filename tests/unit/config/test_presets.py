...
def test_create_preset(tmp_path):
    ...
    with patch("sage.config.presets.get_json_file", return_value=presets_file):
        presets.create("workout", "1hr 30m")
        all_presets = presets.load_all()
...
def test_preset_getter(tmp_path):
    ...
    with patch("sage.config.presets.get_json_file", return_value=presets_file):
        presets.create("break", "5m")
        assert presets.get("break") == {"hours": 0, "minutes": 5, "seconds": 0}
...
def test_rename_preset(tmp_path):
    ...
    with patch("sage.config.presets.get_json_file", return_value=presets_file):
        presets.rename("pomodoro", "beans")
        assert presets.get("beans") == {"minutes": 25}
        assert presets.get("pomodoro") is None
...
def test_update_preset_return_value(tmp_path):
    """
    Test that an updated preset returns the correct preset.
    """
    presets_file = tmp_path / "presets.json"
    with patch("sage.config.presets.get_json_file", return_value=presets_file):
